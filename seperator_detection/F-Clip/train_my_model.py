...
def train_model(device_index):
    args = {'--devices': '0',
     '--help': False,
     '--identifier': 'HG2_LB',
     '<yaml-config>': 'config/newseye_fi.yaml'}
    args['--devices'] = device_index
    ...
    train_loader = torch.utils.data.DataLoader(
        WireframeDataset(datadir, split="train", dataset=dataname), batch_size=M.batch_size, shuffle=True,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--device_index", default='2')
    args = parser.parse_args()
    train_model(args.device_index)
